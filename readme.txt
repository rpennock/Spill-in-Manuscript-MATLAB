...
4) CaTPeakToPeak.m
    ...
    -Average line scan images from images processed using 'AlignImages.m' should be used in this analysis
    -The user will select the number of peaks present in the image, then use a cursor to define the range on the x axis in which the peaks
    are located
    -Fit parameters will be saved as .txt files

5) Cat_Localization.m
    -In imageJ z stack images of cells where a CF CaT was located were flattened using a z projection. A line was then drawn from the soma
    to the CaT site and the distance and angle of the line was mapped.
    -The above data was entered into a spreadsheet and saved as a .txt file
    -This code plots the location of the soma and CaT site relative to the PCL
    -Organization of the spreadsheet columns is indicated in the comments within the code

6) IntegrateFluorProfile.m
    -Plots cumulative sum of fluorescence along a user defined scan for the CF and PF pathway an compares the two using a KS test
    -This script uses the .m files produced by 'CalculateTransients.m' and performs the KS test using the average of the succesful sweeps